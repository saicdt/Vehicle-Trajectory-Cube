...
std::string stringToBeCompressed=msg_content;
...
//boost::iostreams::copy(std::stringstream("hello"), compressingStream);    
...
boost::iostreams::close(compressingStream);
    ...
    << "Content-Encoding: gzip"         << sep
    << "Access-Control-Allow-Origin: *" << sep
//    << "Content-Length: %d"             << sep << sep
//    << "%s";
<< "Content-Length:" << compressedString.size() << sep << sep    << compressedString ; 
   
    // response_size = 106 + (int) msg_content.size(); // banchmark data transfer
    response_size = (int) msg_content.size();
    // check how a binary stream would work here
//    mg_printf(conn, ss.str().c_str(), (int) msg_content.size(), msg_content.c_str());
 mg_write(conn, (char*)ss.str().c_str(), ss.str().size());
